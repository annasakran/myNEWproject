...
//
    ...
    void printselectedexam(ActionEvent event) throws IOException, InterruptedException {
    	...
    	Stage showStage=new Stage();
    	Pair<Parent, Object> displayPair=App.getFxmlAndController("DisplayContent");
    	ControlsubWindow.printExam(Integer.parseInt(id_exam),(DisplayContentController)displayPair.getValue());
    	...
    	{
    		String  mystring="";
    		int personId=App.getPerson_Id();
    		//
			MsgToServer massageMsgToServer = new MsgToServer("Student", "Get",personId);
			try {
				SimpleChatClient.getClient().sendToServer(massageMsgToServer);
			} catch (IOException e) {
				System.out.println("msg not sent ");
			...
			Thread.sleep(1000);
			Student student = App.getStudent_();
			if(student!=null) {
    		
    				List<AnswerSheet> allAnswerSheets=student.getAllAnswerSheets();
    				for(AnswerSheet aSheet:allAnswerSheets)
    				{
    					mystring+="Exam Course : " +aSheet.getExam().getCourseOfExam().getCourseName()+"      Exam ID : "+Integer.toString(aSheet.getExam().getExam_id())+".\n";
    				}
    				show.getAllExamId().setText(mystring);
		         	return;
