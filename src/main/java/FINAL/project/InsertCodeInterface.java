...
public class InsertCodeInterface {
	...
	void EnterCode(ActionEvent event) throws IOException, InterruptedException {
		...
		if (TakeExam.CheckIfLegal(textField.getText()) == 0) {

			// correct code & online exam
			App.setExamCode(textField.getText());
			App.setRoot("OnlineExamInterface");
			
		} else if (TakeExam.CheckIfLegal(textField.getText()) == 1) {
			// correct code & manual exam
			App.setExamCode(textField.getText());
			App.setRoot("ManualExam");
			System.out.println("Going to manual");
			
		} else if (TakeExam.CheckIfLegal(textField.getText()) == -4) {
			App.setCenter("This Exam is over!");
			counter++;
		} 
		...
		// invalid code
			...
			if (counter > 4) {
				...
				WrongInput.setText("X");
				counter++;
				System.out.println("Cont2"+ counter);
			}
		}
		}
	}
