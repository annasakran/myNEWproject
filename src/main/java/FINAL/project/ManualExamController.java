...
public class ManualExamController {
	...
	//	mystartExam.addManual(manualExam);
...
public class ManualExamController {
	...
	void DownloadButton(ActionEvent event) throws IOException, InterruptedException {
		download.setDisable(true);
		MsgToServer massageMsgToServer = new MsgToServer("StartExam", "Get", App.getCode());
		try {
		...
		Thread.sleep(1000);
		mystartExam = App.getStartExam_();
		App.setStartExam_(null);
		System.out.println("mystartExam" + mystartExam.getTeacher());
		TimeLeft = mystartExam.getRemainingTime();

		massageMsgToServer = new MsgToServer("Student", "Get", App.getPerson_Id());
		try {
			SimpleChatClient.getClient().sendToServer(massageMsgToServer);
		} catch (IOException e) {
			System.out.println("msg not sent ");
...

		{
			...
			massageMsgToServer = new MsgToServer("StartExam", "Get", App.getCode());
			try {
				SimpleChatClient.getClient().sendToServer(massageMsgToServer);
			} catch (IOException e) {
				System.out.println("msg not sent ");
			...
			Thread.sleep(1000);
			Exam currentExam = App.getStartExam_().getExam();
			App.setStartExam_(null);
			TimeLeft = mystartExam.getRemainingTime();
			// *****save start exam change
			String theExam = "";
	...
	private void closeExam() throws InterruptedException {
		...
		MsgToServer massageMsgToServer1 = new MsgToServer("", "Save", mystartExam);
		MsgToServer massageMsgToServer2 = new MsgToServer("", "Save", manualExam);
		try {
			SimpleChatClient.getClient().sendToServer(massageMsgToServer1);
			SimpleChatClient.getClient().sendToServer(massageMsgToServer2);
		} catch (IOException e) {
